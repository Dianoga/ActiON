...
Action.Device = Backbone.Model.extend({
	initialize: function() {
		this.set("device_id", this.get("id")), this.set("id", this.get("type") + "_" + this.get("id")), this.set("updating", !1), this.primary && (this.listenTo(this, "change:" + this.primary, this.setStatus), this.setStatus())
	},
	setStatus: function() {
	...
	sendCommand: function(e, t) {
		var i = this;
		i.set("updating", !0);
		var n = i.get("id").split("_")[1],
			t = t || i.get("type");
		Action.sendCommand(n, t, e, function() {
			Action.config.pusher_app || Action.updateData()
		})
	}
	...
	initialize: function() {
		this.listenTo(this, "add", function(e) {
			Action.devices.add(e)
		}), this.listenTo(this, "remove", function(e) {
			Action.devices.remove(e)
		})
	}
	...
	model: Action.Lock
}), Action.Link = Action.Device.extend(), Action.Links = Action.DeviceTypes.extend({
	model: Action.Link
}), Action.Momentary = Action.Device.extend(), Action.Momentaries = Action.DeviceTypes.extend({
	...
	initialize: function() {
		this.set("id", "weather"), this.set("device_id", "weather"), this.setup(), this.listenTo(this, "change:status", this.setup)
	},
	setup: function() {
		var e = this.get("status");
		this.set(_.extend(e.conditions, e.astronomy)), this.set("location", this.get("display_location").full), this.set("skycon", this.weatherIcons[this.get("icon")]);
		var t = this.get("sunrise"),
			i = this.get("sunset");
		this.set("sunrise", t.hour + ":" + t.minute + " AM"), this.set("sunset", i.hour - 12 + ":" + i.minute + " PM")
	}
	...
	getTemplate: function() {
		var e = "#_st-" + this.model.get("type");
		return 0 === $(e).length && (e = "#_st-device"), e
	},
	bindings: {
		".st-title": "name",
		".fa": {
	...
	icons: {},
	onRender: function() {
		this.stickit(), this.listenTo(this.model, "change:updating", function() {
			var e = this.model.get("updating");
			this.$el.toggleClass("updating", e)
		})
	},
	getIcon: function(e, t) {
		_.each(this.icons, function(i, n) {
			e.toggleClass(i, n == t)
		})
	}
	...
	initialize: function() {
		...
		}), this.events["change input"] = this.dimmerChange, this.events["click input"] = function(e) {
			e.stopPropagation()
		}
	},
	dimmerChange: function(e) {
		e.stopPropagation(), this.model.sendCommand(this.model.get("level"))
	}
	...
	changeMode: function(e) {
		this.model.sendCommand($(e.currentTarget).data().mode, "mode")
	},
	changePhrase: function(e) {
		this.model.sendCommand($(e.currentTarget).data().phrase, "hellohome")
	}
	...
	$.ajax({
		...
		success: function(e) {
			console.log(e), Action.contacts.set(new Action.Contacts(e.contacts).models), Action.dimmers.set(new Action.Dimmers(e.dimmers).models), Action.humidities.set(new Action.Humidities(e.humidity).models), Action.locks.set(new Action.Locks(e.locks).models), Action.links.set(new Action.Links(e.links).models), Action.momentaries.set(new Action.Momentaries(e.momentary).models), Action.motions.set(new Action.Motions(e.motion).models), Action.presences.set(new Action.Presences(e.presence).models), Action.switches.set(new Action.Switches(e.switches).models), Action.temperatures.set(new Action.Temperatures(e.temperature).models);
			var t = new Action.Weather(e.weather),
				i = new Action.Mode(_.extend(e.hellohome, e.mode)),
				n = {
					at: 0,
					merge: !0
				};
			Action.devices.add(i, n), Action.devices.add(t, n), Action.config.pusher_app ? Action.setupPusher() : Action.config.refresh && (Action.refreshTimeout = _.delay(Action.updateData, 60 * Action.config.refresh * 1e3))
		}
	})
}, Action.setupPusher = function() {
	var e = new Pusher(Action.config.pusher_app),
		t = e.subscribe("devices");
	t.bind("device_update", Action.pusherDeviceUpdate)
}, Action.pusherDeviceUpdate = function(e) {
	var t = Action.devices.findWhere({
		device_id: e.id
	});
	t && (t.set(e.name, e.value), t.set("updating", !1))
}, Action.sendCommand = function(e, t, i, n) {
	$.ajax({
